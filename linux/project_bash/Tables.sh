...
function list_tables() {
    ...
    if [[ -z "$(ls -A "$DB_PATH/$dbname" | grep -v '^\.' )" ]]; then
    ...
    for line in "${cols_meta[@]}"; do
        ...
        while true; do
            ...
            if [[ "$col_type" == "int" && ! "$value" =~ ^[0-9]+$ ]]; then
                ...
                continue
            elif [[ "$col_type" == "string" && "$value" == *:* ]]; then
            ...
            if [[ "$is_pk" == "yes" ]]; then
                ...
                if ! awk -F: -v val="$value" -v idx="$col_index" '{if ($idx==val) exit 1} END {exit 0}' "$table_path"; then
...
function select_from_table() {
    ...
    # Print headers
    awk -F: '{printf "%-15s", $1} END {print ""}' "$meta_path"
    # Print rows
    awk -F: '{
...
function delete_from_table() {
    ...
    if awk -F: -v idx="$pk_index" -v val="$pk_val" '$idx == val {found=1} END {exit !found}' "$table"; then
        awk -F: -v idx="$pk_index" -v val="$pk_val" '$idx != val' "$table" > "$table.tmp" && mv "$table.tmp" "$table"
        echo "Row deleted."
    else
...
function update_table() {
    ...
    local line_num=$(awk -F: -v idx="$pk_index" -v val="$pk_val" '{if ($idx == val) {print NR; exit}}' "$table")
    ...
    awk -v line="$line_num" -v idx="$col_choice" -v new="$new_val" -F: 'BEGIN{OFS=":"}{if (NR == line) $idx = new; print}' "$table" > "$table.tmp" && mv "$table.tmp" "$table"
